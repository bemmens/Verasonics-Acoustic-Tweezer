...
TW(1).Parameters = [Trans.frequency,1,100,1]; % A, B, C, D
TW(1).equalize = 0;
...
% Left/Right Steering
nThetaPSweep = 40; 
thetas = linspace(0,pi/3,nThetaPSweep);
na = length(thetas);
...
TTNB = SeqControl(3).argument*1e-6;
...
sweepTime = 15;
nTXpSweep = ceil(sweepTime/TTNB);
%nTXpSweep = 85e3/2; %based on 30s sweep
nTXpTheta = ceil(nTXpSweep/nThetaPSweep);

holdTime = 0;
nTXpHold = ceil(holdTime/TTNB);

totalSequenceTime = 0;
% Centre hold
angleIdx = 1;
for i = 1:round(nTXpHold/2)
    Event(n).info = 'Transmit'; 
    Event(n).tx = 1; % first theta is the centre
    Event(n).rcv = 0; 
    Event(n).recon = 0; 
    Event(n).process = 0; 
...
totalSequenceTime = totalSequenceTime + TTNB*nTXpHold;

% Centre to outer edge
angleIdx = 1;
for i = 1:nThetaPSweep
    for singlethetaTXs = 1:round(nTXpTheta/2)
        Event(n).info = 'Transmit'; 
        Event(n).tx = angleIdx; 
        Event(n).rcv = 0; 
        Event(n).recon = 0; 
...
totalSequenceTime = totalSequenceTime + TTNB*nThetaPSweep*nTXpTheta;

% Edge hold
angleIdx = 1;
for i = 1:round(nTXpHold/2)
    Event(n).info = 'Transmit'; 
    Event(n).tx = na; % first theta is the centre
    Event(n).rcv = 0; 
    Event(n).recon = 0; 
    Event(n).process = 0; 
    Event(n).seqControl = 3; 
...
save('Barney/data_files/Vortex_Ring_Sweeping'); 
disp('KerCHOW!')
%EventAnalysisTool
%VSX
