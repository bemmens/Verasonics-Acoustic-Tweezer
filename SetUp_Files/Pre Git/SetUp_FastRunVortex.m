...
Event = genEventSeq(na,TxSlider);

% Create UI Controls
import vsv.seq.uicontrol.VsSliderControl
UI(1).Control = VsSliderControl('LocationCode','UserA1',...
                 'Label','Steering Angle',... % Now in degrees
                 'SliderMinMaxVal',[0,max(rad2deg(thetas)),0],... % min,max,initial
                 'SliderStep', [1/(length(thetas)-1),2/(length(thetas)-1)]);   
UI(1).Callback = @defSteering;

% Save all the structures to a .mat file.
save('Barney/data_files/FastRunVortex'); 

% Callback timing
Control.Command = 'update&Run';
Control.Parameters = {'Event'};
f = @() defSteering(1,1,rad2deg(thetas(2)));
...
function out = defSteering(~,~,TxAngle)
    % TxAngle in deg
    % Thetas in rad
    %TxAngle
    %deg2rad(TxAngle)
    thetas = evalin('base','thetas');
    %TxIdxs = find(thetas<deg2rad(TxAngle));
    [roundingError,TxIdx] = min(abs(thetas-deg2rad(TxAngle)));
    assignin('base',"TxIdx",TxIdx) ;
...
function Event = genEventSeq(na,TxSlider)
n=1;
