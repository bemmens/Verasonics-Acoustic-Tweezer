...
PRF = 250; % Hz
TTNB = 1e6/PRF; % us
%TTNB = 1000;  % us pause between pulses (10 is minimum specifiable)
SeqControl(3).command = 'timeToNextAcq';
SeqControl(3).argument = TTNB;
SeqControl(4).command = 'returnToMatlab';

if TTNB < burstLength
    fprintf(2,'ERROR: TW longer that TTNB!\n')
end
...
%% Print TX Properties
rechargeTime = TTNB - burstLength;
PRF = 1/(TTNB*1e-6);
DC = genDC(genWvfm(TW(1).Parameters),TTNB) *100; % in %

disp('--- Pulse Stats ---')
disp(strcat('Number of Cycles:',string(nHalfCycles/2)))
disp(strcat('Burst Length: ', string(burstLength), ('us')))
disp(strcat('Recharge Time: ',string(rechargeTime), 'us'))
disp(strcat('TTNB: ',string(TTNB), 'us'))
disp(strcat('Time Per Callback:',string(usPerCallback),'us'))
disp(strcat('PRF:',string(round(PRF,0)),'Hz'))
disp(strcat('Max Voltage: ',string(TPC(5).maxHighVoltage),'V'))
disp(strcat('Duty Cycle:', string(DC),'%'))
disp('-------------------')

plot(genWvfm(TW(1).Parameters))
%% Create UI Controls
